...
- Bump the version so VS Code re-reads the contribution points. Profiles that
  already had 0.0.1 installed kept the cached activity bar layout and never
  showed the new container.

### [0.0.1]

Initial fork of Cline 4.0.0.

**Added**

- Activity bar icon that reveals the chat view in the secondary (right) side bar.

**Changed**

- Rebranded to OpenProvider: extension identity, `openprovider.*` command
  namespace, view containers, icons, walkthrough and user-facing text. The OAuth
  redirect URI moved to `vscode://openprovider.openprovider`, which VS Code
  requires to match the extension's publisher and name.
- The chat view now lives in the secondary side bar rather than the activity bar.
- Onboarding offers bring-your-own-key only. The "Absolutely Free", "Frontier
  Model" and ClinePass paths all required a Cline-hosted account.

**Removed**

- The Cline-hosted account, ClinePass, credit balance and billing UI, and the
  org-level remote-config subsystem, all of which depended on Cline's backend.
  `AuthService` was kept and stripped in place: the OpenRouter, Requesty, Hicap,
  Codex and OCA OAuth flows still work.
- PostHog telemetry, error reporting and feature flags, which reported to
  Cline's `data.cline.bot` proxy. Self-hosted OpenTelemetry export is untouched.
- Cline-specific Discord, docs and issue-template links.

**Fixed**

- The sidebar rendered blank because `BannerService.initialize()` had been called
  as a side effect of the removed account flow, so `getStateToPostToWebview()`
  threw and the webview never received state. Banners are now optional there, so
  a failure cannot take down the whole UI again.
- The activity bar icon rendered as a solid square: it embedded a raster image,
  and VS Code draws these icons as a single-colour mask.

---

## Upstream Cline

The history below is Cline's, from before this fork. It is kept for provenance;
the versions do not correspond to OpenProvider releases.
