...
import { CLINE_ACCOUNT_AUTH_ERROR_MESSAGE } from "@shared/cline/api"
...
export class Controller {
	...
	constructor(readonly context: ClineExtensionContext) {
		...
		// Initialize SDK-backed auth service (OAuth infra for OpenRouter/Requesty/
		// Hicap/OpenAI Codex — Cline-account auth/billing has been removed).
	...
	/**
	 * Remote config (organization policy pushed from Cline's backend) has been
	 * removed along with the Cline account/billing system this fork does not
	 * use — there is no backend to fetch it from. This refresh method is kept
	 * as a no-op rather than removed, since call sites throughout this file
	 * (task start, sign-out, etc.) still reference it.
	 */
	private async refreshRemoteConfig(): Promise<void> {
		// No-op: remote config backend removed.
	}
	...
	/**
	 * Cline-account sign-out has been removed along with the account/billing
	 * system this fork does not use. Kept as a no-op so the gRPC RPC surface
	 * (accountLogoutClicked) still compiles and resolves without regenerating
	 * protos.
	 */
	...
	/**
	 * Cline-account OAuth callback handling has been removed along with the
	 * account/billing system this fork does not use. Kept as a no-op so the
	 * URI handler (SharedUriHandler) still compiles and resolves without
	 * regenerating protos.
	 */
