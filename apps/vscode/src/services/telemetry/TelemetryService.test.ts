...
describe("Telemetry system is abstracted and can easily switch between providers", () => {
	...
	describe("Factory Configuration", () => {
		it("should return default configurations", () => {
			...
			assert.ok(defaultConfigs.length > 0, "Should return at least one configuration (falls back to no-op)")

			isSelfHostedStub.restore()
		})

		it("should never include PostHog config, regardless of selfHosted mode", () => {
			// PostHog (Cline's first-party data.cline.bot proxy) has been removed
			// entirely from this fork -- it has no PostHog project of its own and
			// must never send data to Cline's.
			for (const selfHosted of [true, false]) {
				const isSelfHostedStub = sinon.stub(ClineEndpoint, "isSelfHosted").returns(selfHosted)
				const configs = TelemetryProviderFactory.getDefaultConfigs()
				const hasPosthog = configs.some((c) => (c as { type: string }).type === "posthog")
				assert.strictEqual(hasPosthog, false, `Should never include PostHog configuration (selfHosted=${selfHosted})`)
				isSelfHostedStub.restore()
			}
		})
