 ...
 *
 * NOTE: PostHog (Cline's first-party data.cline.bot proxy) has been removed
 * as a provider type -- this fork must never send telemetry to Cline's
 * PostHog project. Self-hosted OpenTelemetry export is untouched.
 */
...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		// PostHog telemetry (Cline's first-party data.cline.bot proxy) has been
		// disabled -- this fork has no PostHog project of its own and must
		// never send data to Cline's. Self-hosted OpenTelemetry export below is
		// untouched and keeps working for users who configure their own
		// OTel endpoint.
