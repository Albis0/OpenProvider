...
export class BannerService {
	...
	private async doFetch(): Promise<Banner[]> {
		...
		try {
			...
			// Cline-account auth has been removed along with the account/billing
			// system this fork does not use, so remote banners are always fetched
			// unauthenticated now (personalized/account-gated banners will not apply).
