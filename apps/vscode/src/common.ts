...
export async function initialize(storageContext: StorageContext): Promise<WebviewProvider> {
	...
	// PostHog (Cline's first-party data.cline.bot proxy) is not used in this fork.
	// No first-party telemetry client is initialized; only user-configured
	// OpenTelemetry export remains available.

	// =============== Webview services ===============
	const webview = HostProvider.get().createWebviewProvider()

	// BannerService must be initialized before the first getStateToPostToWebview()
	// call, which reads active banners. Upstream initialized it as a side effect of
	// the Cline-account auth flow; that flow is gone in this fork, so it is
	// initialized here instead. StateManager.initialize() above has already set up
	// HostRegistryInfo, which BannerService.initialize() requires.
	try {
		BannerService.initialize(webview.controller)
	} catch (error) {
		// Never let banners break startup: without this, a throw here would
		// propagate into the state subscription and leave the webview blank.
		Logger.error("[BannerService] Failed to initialize", error)
	}
		...
		if (!previousVersion || currentVersion !== previousVersion) {
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `OpenProvider has been updated to v${currentVersion}`
					: `Welcome to OpenProvider v${currentVersion}`
				HostProvider.window.showMessage({
