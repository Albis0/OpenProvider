...
import { VscodeLauncherViewProvider } from "./hosts/vscode/VscodeLauncherViewProvider"
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// The view lives in the secondary (right) side bar; see the `secondarySidebar`
	// viewsContainers contribution in package.json, which requires VS Code >= 1.106
	// (enforced by the engines field).
	...
	// Activity bar icon: clicking it reveals the chat view in the secondary side bar.
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(VscodeLauncherViewProvider.VIEW_ID, new VscodeLauncherViewProvider()),
	)
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "openprovider.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[OpenProvider Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log(`[OpenProvider Dev] Failed to register dev commands: ${error}`)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to OpenProvider (Always available)
					const addAction = new vscode.CodeAction("Add to OpenProvider", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: commands.AddToChat,
						title: "Add to OpenProvider",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with OpenProvider (Always available)
					const explainAction = new vscode.CodeAction(
						"Explain with OpenProvider",
						vscode.CodeActionKind.RefactorExtract,
					) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with OpenProvider",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with OpenProvider (Always available)
					const improveAction = new vscode.CodeAction(
						"Improve with OpenProvider",
						vscode.CodeActionKind.RefactorRewrite,
					) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with OpenProvider",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with OpenProvider (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with OpenProvider", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithCline,
							title: "Fix with OpenProvider",
							arguments: [expandedRange, context.diagnostics],
						}
...
\`\`\``
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand(
				...
				`${context.extension.id}#OpenProviderWalkthrough`,
			)
	...
	Logger.log(`[OpenProvider] extension activated in ${performance.now() - activationStartTime} ms`)
...
function setupHostProvider(context: ExtensionContext) {
	...
	outputChannel.appendLine("[OpenProvider] Setting up VS Code host...")
...
async function openClineSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for OpenProvider sidebar visibility")
}
