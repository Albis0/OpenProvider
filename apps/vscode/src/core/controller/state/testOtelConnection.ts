...
export async function testOtelConnection(_controller: Controller, _: EmptyRequest): Promise<TestConnectionResult> {
	try {
		...
		// Remote-config-pushed OTel (organization policy from Cline's backend) has
		// been removed along with the account/billing system this fork does not
		// use. Test against the user's own self-hosted OTel provider instead,
		// which uses the default provider name.
		const otelProvider = providers.find((p) => p.name === "OpenTelemetryProvider")
