...
export async function getStateToPostToWebview(controller: {
	...
	// Banners are non-essential chrome. If the service is unavailable, fall back to
	// empty lists rather than throwing: this function feeds the webview's state
	// subscription, and a throw here leaves the whole UI unrendered.
	let banners: ReturnType<BannerService["getActiveBanners"]> = []
	let welcomeBanners: ReturnType<BannerService["getWelcomeBanners"]> = []
	try {
		const bannerService = BannerService.get()
		banners = bannerService.getActiveBanners() ?? []
		welcomeBanners = bannerService.getWelcomeBanners() ?? []
	} catch (error) {
		Logger.error("[getStateToPostToWebview] Banners unavailable", error)
	}
