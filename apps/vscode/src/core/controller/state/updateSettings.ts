...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		...
		if (request.optOutOfRemoteConfig !== undefined) {
			// Remote config (organization policy pushed from Cline's backend) has
			// been removed along with the Cline account/billing system this fork
			// does not use. The opt-out preference is still stored so the webview
			// setting round-trips correctly, but there is nothing to clear or fetch.
			controller.stateManager.setGlobalState("optOutOfRemoteConfig", !!request.optOutOfRemoteConfig)
		}
